...
export default function RAMInstallationGame({ onComplete, onClose, fasterUpgrade }) {
  const [ramInstalled, setRamInstalled] = useState(false);

  const installRam = () => {
    setRamInstalled(true);
    // Kalo punya Obeng Elektrik, instalasi instan. Kalo ngga, nunggu 1 detik.
    setTimeout(() => {
      onComplete();
    }, fasterUpgrade ? 0 : 1000);
  };
  ...
  return (
    <>
      ...
      {fasterUpgrade && <Text style={{color: '#a3e635', marginBottom: 10}}>*Obeng Elektrik: Instalasi Instan!</Text>}
